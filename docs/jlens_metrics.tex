...
Why it matters:
\begin{itemize}
  \item \textbf{Workspace-onset diagnostic:} The workspace hypothesis predicts $\kappa_\ell$ \emph{low} in early (sensory) layers, rising sharply at the workspace ``ignition'' layer, and $\kappa_\ell \approx 1$ in the motor band. A flat $\kappa_\ell$ profile would falsify the workspace claim.
  \item \textbf{On Qwen3.5-4B:} $\kappa$ rises from $\approx 0.41$ (layer~4) to $\approx 0.97$ (layer~30), with the fluctuation-to-coherent energy ratio falling by two orders of magnitude. Six layers cannot distinguish a sharp onset from a gradual drift; the full-depth curve is a key open measurement.
  \item \textbf{On Pythia-160M across training:} $\kappa_0$ drops from 0.94 (step~1) to 0.37 (step~143,000)---the opposite of the depth profile! At initialization, $\kappa$ is high everywhere because $J \approx I$ always (trivial agreement). As training proceeds, per-prompt Jacobians \emph{diverge}, and the mean becomes less representative. $\kappa_0$ is strongly anti-correlated with downstream performance ($\rho = -0.84$ with ARC-Easy).
  \item \textbf{$\kappa$ at late training vs.\ $\kappa$ at initialization are different things:} In a trained model, low $\kappa$ in early layers = structured, prompt-specific transport. At initialization, high $\kappa$ = trivial identity transport. The \emph{shape} of $\kappa(\ell)$ (flat at init, structured at convergence) is the developmental signature.
\end{itemize}
