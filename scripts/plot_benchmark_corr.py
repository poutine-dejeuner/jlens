...
fig1.suptitle("Spearman ρ: J-lens metrics vs Benchmark scores\n(Pythia-160M deduped, 26 checkpoints)",
...
corr_matrix = np.zeros((len(plot_metrics), len(benchmarks)))
...
for i, mkey in enumerate(plot_metrics):
    ...
    for j, bm in enumerate(benchmarks):
        ...
        if mask.sum() >= 5:
            r, p = stats.spearmanr(mvals[mask], bvals[mask])
            corr_matrix[i, j] = r
            p_matrix[i, j] = p

im = ax1.imshow(corr_matrix, cmap="RdBu_r", aspect="auto", vmin=-1, vmax=1)
ax1.set_xticks(range(len(benchmarks)))
ax1.set_xticklabels(bm_labels, rotation=45, ha="right", fontsize=10)
ax1.set_yticks(range(len(plot_metrics)))
ax1.set_yticklabels(plot_metrics, fontsize=9)
ax1.set_title("Spearman rank correlation (ρ)", fontsize=11)
    ...
    for j in range(len(benchmarks)):
        ...
        if not np.isnan(r):
            ...
            ax1.text(j, i, f"{r:.2f}{sig}", ha="center", va="center",
                     fontsize=9, color=color, fontweight="bold" if abs(r) > 0.7 else "normal")

cbar = plt.colorbar(im, ax=ax1, shrink=0.8)
cbar.set_label("Spearman ρ", fontsize=10)

fig1.tight_layout()
fig1.savefig("figures/correlation_heatmap.png", dpi=200, bbox_inches="tight")
plt.close(fig1)
print("Saved figures/correlation_heatmap.png")

# ── Figure 2: Best scatter plots ──────────────────────────────────────
# Top 3 best-correlated metric-benchmark pairs
top_pairs = [
    ("a_coeff_L0", "arc_easy", "Identity coefficient a (L0)"),
    ("coherence_L0", "piqa", "Coherence κ (L0)"),
...
for idx, (mkey, bm, mlabel) in enumerate(top_pairs):
    ...
    ax.set_title(f"ρ = {r_s:.3f}**, r = {r_p:.3f}, n = {len(x)}",
...
# Find common steps with both metrics
mkey = "a_coeff_L0"
bm = "arc_easy"
mvals = jlens_metrics[mkey]
bvals = eval_data[bm]
...
color_jlens = "#2196F3"  # blue
color_eval = "#FF5722"   # deep orange

# Left axis: a_coeff (decreasing = model learns)
...
ax1.text(0.02, 0.95, f"Spearman ρ = {r_s:.3f}**\nn = {len(s)}",
...
# ── Figure 4: All benchmarks vs top J-lens metrics as small multiples ──
fig4, axes = plt.subplots(3, 3, figsize=(18, 18), constrained_layout=True)
fig4.suptitle("J-lens metrics vs Benchmark scores — full correlation matrix",
...
# Flatten the benchmarks we care about (exclude LogiQA, WSC — weak correlations)
...
for row_idx, mkey in enumerate(top_metrics[:3]):
    for col_idx, bm in enumerate(plot_bms[:3]):
        ax = axes[row_idx, col_idx] if len(top_metrics) > 1 else axes[col_idx]
        mvals = jlens_metrics[mkey]
        bvals = eval_data[bm]
        mask = ~np.isnan(mvals) & ~np.isnan(bvals)
        x, y = mvals[mask], bvals[mask]
        if len(x) >= 5:
            ...
            ax.set_title(f"ρ = {r_s:.3f}{'**' if p_s < 0.01 else '*' if p_s < 0.05 else ''}",
                         fontsize=10)
...
for mkey in jlens_metrics:
    ...
    for bm in benchmarks:
        ...
        if mask.sum() >= 5:
            r, p = stats.spearmanr(mvals[mask], bvals[mask])
            if abs(r) > 0.80:
                results.append((abs(r), r, p, mkey, bm))

results.sort(reverse=True)
for abs_r, r, p, mkey, bm in results:
    sig = "**" if p < 0.01 else "*"
    print(f"  ρ = {r:+.4f}{sig}  |  {mkey:25s}  ×  {bm}")
